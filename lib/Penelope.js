...
function Penelope(metrics) {
    this.metrics = metrics;
}

Penelope.prototype.run = function () {
    var args = arguments;
...
var runMetrics = function (metrics, stylesheets) {
    ...
    _.each(stylesheets, function (rawStylesheet) {
        ...
        var runMetricsOnRule = function (rawRule, parentRules) {
            if (!_.isArray(parentRules)) {
                parentRules = [];
            }
            ...
            readings.push(runMetricsOnNode(metrics, rawRule, 'rule', {parentRules: parentRules, childRules: childRules}));

            if (rule.getRules().length) {
                _.each(rule.getRules(), function (rawChildRule) {
                    runMetricsOnRule(rawChildRule, parentRules.concat([rawRule]));
                });
            }
...
var runMetricsOnNode = function (metrics, node, type, data) {
    var measurements = {};
    _.each(filterMetricsByType(metrics, type), function (metric) {
        var measurement = metric.measure(node, data);
        if (!_.isArray(measurement)) {
            measurement = [measurement];
        }

        measurements[metric.id] = measurement;
    });
